...
def faceRecognitionPipeline(filename,path=True):
    ...
    for x,y,w,h in faces:
        ...
        roi_mean = roi_reshape - mean_face_arr # subtrack face with mean face
        ...
        print(text)
        # defining colour based on results
        if results[0] == 'male':
            color = (255,255,0)
        else:
            color = (255,0,255)
        ...
        cv2.putText(img,text,(x,y),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),5)
        output = {
            'roi': roi,
            'eig_img': eig_img,
